...
export function generateCursor(ctx: GenContext): PlannedFile[] {
  ...
  files.push(
    pf(
      ...
      mdc("How savings are measured", "**/*", `# Measurement\n\nToken usage is tracked via \`ccusage\`. Run \`npx agent-stack measure --since 7d\` to see savings.`),
    ),
  );
...
_Generated by agent-stack. Edit \`CLAUDE.md\` and run \`npx agent-stack sync\` to regenerate this mirror._
`,
    ),
  );
